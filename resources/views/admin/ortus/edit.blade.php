...
<div class="row">
    <div class="col-12">
        <div class="card card-dashboard">
            ...
            <div class="card-body">
                <form action="{{ route('admin.ortus.update', $ortu->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                   id="name" name="name" value="{{ old('name', $ortu->name) }}" required>
                            @error('name')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="email" class="form-control @error('email') is-invalid @enderror" 
                                   id="email" name="email" value="{{ old('email', $ortu->email) }}" required>
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('phone') is-invalid @enderror" 
                                   id="phone" name="phone" value="{{ old('phone', $ortu->phone) }}" required>
                            @error('phone')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select class="form-select @error('student_id') is-invalid @enderror" 
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" 
                                        {{ (old('student_id', $ortu->students->first()->id ?? '') == $student->id) ? 'selected' : '' }}>
